...
from runloop_api_client import Runloop, BaseModel, AsyncRunloop
...
from runloop_api_client._base_client import FinalRequestOptions
...
def test_extract_response_type_direct_class_missing_type_arg() -> None:
    with pytest.raises(
        RuntimeError,
        match="Expected type <class 'runloop_api_client._response.AsyncAPIResponse'> to have a type argument at index 0 but it did not",
    ):
...
def test_response_parse_mismatched_basemodel(client: Runloop) -> None:
    ...
    with pytest.raises(
        TypeError,
        match="Pydantic models must subclass our base model type, e.g. `from runloop_api_client import BaseModel`",
    ):
        response.parse(to=PydanticModel)


@pytest.mark.asyncio
async def test_async_response_parse_mismatched_basemodel(async_client: AsyncRunloop) -> None:
...
def test_response_parse_custom_stream(client: Runloop) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=client,
        stream=True,
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_stream(async_client: AsyncRunloop) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=b"foo"),
        client=async_client,
        stream=True,
...
def test_response_parse_custom_model(client: Runloop) -> None:
    response = APIResponse(
...
@pytest.mark.asyncio
async def test_async_response_parse_custom_model(async_client: AsyncRunloop) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=async_client,
        stream=False,
...
def test_response_parse_annotated_type(client: Runloop) -> None:
    response = APIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=client,
        stream=False,
...
async def test_async_response_parse_annotated_type(async_client: AsyncRunloop) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=json.dumps({"foo": "hello!", "bar": 2})),
        client=async_client,
        stream=False,
...
def test_response_parse_bool(client: Runloop, content: str, expected: bool) -> None:
    response = APIResponse(
...
async def test_async_response_parse_bool(client: AsyncRunloop, content: str, expected: bool) -> None:
    response = AsyncAPIResponse(
        raw=httpx.Response(200, content=content),
        client=client,
        stream=False,
