...
# Test constants
SHORT_SLEEP = 0.1  # Brief pause for thread startup
MEDIUM_SLEEP = 0.2  # Slightly longer pause
...
class TestExecution:
    ...
    def test_init(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
        ...
        execution = Execution(mock_client, "dev_123", execution_view)
        assert execution.execution_id == "exec_123"
        assert execution.devbox_id == "dev_123"
        assert execution._latest == execution_view

    def test_init_with_streaming_group(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
        """Test Execution initialization with streaming group."""
        threads = [threading.Thread(target=lambda: None)]
        stop_event = threading.Event()
        streaming_group = _StreamingGroup(threads, stop_event)

        execution = Execution(mock_client, "dev_123", execution_view, streaming_group)
        assert execution._streaming_group is streaming_group

    def test_properties(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
        """Test Execution properties."""
        execution = Execution(mock_client, "dev_123", execution_view)
        assert execution.execution_id == "exec_123"
        assert execution.devbox_id == "dev_123"

    def test_result_already_completed(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
    ...
    def test_result_needs_polling(self, mock_client: Mock) -> None:
        ...
        execution = Execution(mock_client, "dev_123", running_execution)
        result = execution.result()
    ...
    def test_result_with_streaming_group(self, mock_client: Mock) -> None:
        ...
        execution = Execution(mock_client, "dev_123", running_execution, streaming_group)
        result = execution.result()

        assert result.exit_code == 0
        assert execution._streaming_group is None  # Should be cleaned up

    def test_get_state(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
    ...
    def test_kill(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
        """Test kill method."""
    ...
    def test_kill_with_process_group(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
    ...
    def test_kill_with_streaming_cleanup(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
    ...
    def test_stop_streaming_no_group(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
        """Test _stop_streaming when no streaming group."""
        execution = Execution(mock_client, "dev_123", execution_view)
        execution._stop_streaming()  # Should not raise

    def test_stop_streaming_with_group(self, mock_client: Mock, execution_view: SimpleNamespace) -> None:
