...
class TestAxonOps:
    ...
    def test_create(self, mock_client: Mock, axon_view: MockAxonView) -> None:
        """Test create method."""
        mock_client.axons.create.return_value = axon_view

        ops = AxonOps(mock_client)
        axon = ops.create(name="test-axon")

        assert isinstance(axon, Axon)
        assert axon.id == "axn_123"
        mock_client.axons.create.assert_called_once()
    ...
    def test_list_empty(self, mock_client: Mock) -> None:
        ...
        mock_client.axons.list.return_value = page

        ops = AxonOps(mock_client)
        axons = ops.list()

        assert len(axons) == 0
        mock_client.axons.list.assert_called_once()

    def test_list_single(self, mock_client: Mock, axon_view: MockAxonView) -> None:
        """Test list method with single result."""
        ...
        assert len(axons) == 1
        assert isinstance(axons[0], Axon)
    ...
    def test_list_multiple(self, mock_client: Mock) -> None:
        ...
        assert len(axons) == 2
        assert isinstance(axons[0], Axon)
        assert isinstance(axons[1], Axon)
