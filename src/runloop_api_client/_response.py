...
from ._exceptions import RunloopError, APIResponseValidationError
...
class APIResponse(BaseAPIResponse[R]):
    ...
    def parse(self, *, to: type[_T] | None = None) -> R | _T:
        ...
        ```py
        from runloop_api_client import BaseModel
...
class MissingStreamClassError(TypeError):
    def __init__(self) -> None:
        super().__init__(
            "The `stream` argument was set to `True` but the `stream_cls` argument was not given. See `runloop_api_client._streaming` for reference",
        )


class StreamAlreadyConsumed(RunloopError):
    """
